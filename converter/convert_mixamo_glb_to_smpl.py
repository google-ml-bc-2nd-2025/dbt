...
def save_smpl_npz(pose, out_path, fps=30, trans=None, betas=None):
    """
    ...
    # NPZ 저장 (포즈, 위치, 형상, FPS 정보)
    np.savez(out_path, poses=pose, trans=trans, betas=betas, fps=fps)
    print(f"SMPL 데이터 저장 완료: {out_path}")

def save_smpl_npy(pose, out_path, fps=30, trans=None, betas=None):
    """
...
if __name__ == "__main__":
    ...
    save_smpl_npy(pose, npy_path, fps=fps)
    
    # NPZ 형식으로도 저장 (여러 배열)
    npz_path = f"{base_name}_smpl.npz"
    save_smpl_npz(pose, npz_path, fps=fps)
    
    print(f"변환 완료: {glb_path} → {npy_path} 및 {npz_path} (총 {frame_count} 프레임, FPS: {fps:.2f})")
